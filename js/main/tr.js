...
function getTimeCubeGain() {
	...
	gain = gain.div(getMinusNerf("cubeGain"));
...
function tr1Eff() {
	return ExpantaNum.pow(1 + 0.1 / getMinusNerf("vanillaTimeUpgrades"), player.rank.plus(player.tier));
}

function tr2Pow() {
	let pow = new ExpantaNum(1)
	if (tmp.pathogens && player.pathogens.unl) pow = pow.times(tmp.pathogens[1].eff());
	pow = pow.times(1 / getMinusNerf("vanillaTimeUpgrades"))
	return pow
}
...
function tr4Eff() {
	...
	return ExpantaNum.pow(1 + 0.33 / getMinusNerf("vanillaTimeUpgrades"), r.plus(1).log10())
}

function tr6Eff() {
	return ExpantaNum.pow(1 + 0.1 / getMinusNerf("vanillaTimeUpgrades"), player.tr.cubes.plus(1).log10())
}

function tr7Eff() {
	return ExpantaNum.pow(1 + 0.05 / getMinusNerf("vanillaTimeUpgrades"), player.achievements.length)
}
...
function tr10Eff() {
	...
	return ExpantaNum.pow(1 + 0.1 / getMinusNerf("vanillaTimeUpgrades"), cubes.plus(1).log10())
}

function tr11Pow() {
	let pow = new ExpantaNum(1 / getMinusNerf("vanillaTimeUpgrades"))
...
function tr12Eff() {
	return tmp.dc ? tmp.dc.allComp.plus(1).sqrt().pow(1 / getMinusNerf("vanillaTimeUpgrades")) : new ExpantaNum(1)
}
...
function tr14Eff() {
	return {
		cd: player.tier.plus(1).pow(1.25).pow(1 / getMinusNerf("vanillaTimeUpgrades")),
		ss: player.dc.cores.plus(1).log10().plus(1).log10().times(7.5).pow(1 / getMinusNerf("vanillaTimeUpgrades"))
	}
}
...
function tr15Eff() {
	...
	return eff.pow(1 / getMinusNerf("vanillaTimeUpgrades"))
}
